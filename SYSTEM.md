...
11. **I decoupled the frontend, backend, and database.**  
    Initially, Streamlit handled the user interface while directly invoking the RAG/agent logic and accessing PostgreSQL. I introduced FastAPI as a dedicated backend for RAG, agent orchestration, and database access, leaving Streamlit solely responsible for the user interface and PostgreSQL/pgvector as the independent data layer. This added API complexity but improved separation of concerns, maintainability, testability, and the ability to support additional clients in the future.

12. **I separated development and runtime dependencies to keep deployment lean.**  
    After Docker builds exhausted the available disk space on my machine, I separated development-only dependencies—such as ingestion, notebooks, evaluation, and document processing—from those required to run the deployed application. This reduced the deployment footprint while allowing the full development environment to retain the tools needed for data processing, experimentation, and evaluation.
