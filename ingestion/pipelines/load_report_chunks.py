...
def main() -> None:
    ...
    chunks = process_all_reports(reports)

    if not chunks:
        raise RuntimeError("No chunks were produced.")

    table_count = sum(bool(chunk["contains_table"]) for chunk in chunks)

    logger.info(
        "Produced %d chunks; %d contain " "probable table content.",
        len(chunks),
        table_count,
    )
    ...
    load_info = pipeline.run(report_chunks_resource(chunks))

    print(load_info)

    logger.info(
        "Loaded chunks into " "%s.report_chunks.",
        DATASET_NAME,
    )
